        ...
        if project == "new":
            if language == "zh":
                self._print(
                    ...
                    "   - 根目录下已存在AGENTS.md（Claude Code根目录下需将AGENTS.md重命名为CLAUDE.md）\n"
                    "   - 对于支持 role-rules 的工具，已经将.harbor/rules/role-rules.md 复制到 role-rules\n"
                    "2. 如需项目专属规则，请让 AI 工具生成：\n"
                    "   - .harbor/rules/project-rules.md\n"
                    "   参考：.harbor/rules/project-rules-guide.md\n"
                    "3. 开始首个 AI coding 任务：\n"
                    "   harbor start\n"
                    "4. 开发完成一个有意义的 AI coding 单元后：\n"
                ...
                self._print(
                    ...
                    "   - AGENTS.md already exists in the root directory (for Claude Code, rename AGENTS.md to CLAUDE.md in the root directory)\n"
                    "   - For tools that support role-rules, copy .harbor/rules/role-rules.md to role-rules\n"
